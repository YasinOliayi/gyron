...
class filters:
    ...
    def private() -> Callable :
        ...
        return lambda message: message.chat_type == 'private'
    
    @staticmethod
    def group():


        return lambda message: message.chat_type == 'group'
    
    @staticmethod
    def channel():

        return lambda message: message.chat_type == 'channel'
    ...
    def new_member():
        ...
        def _filter(update:Update):
            ...
            if update.new_chat_members is not None:
                return True
    ...
    def left_member():
        ...
        return lambda message: message.left_chat_member is not None
    ...
    def is_joined(*chat_ids: Union[str, int]):
        ...
        if not chat_ids:
            ...
            raise ValueError("The 'is_joined' filter requires at least one chat ID.")

        for chat_id in chat_ids:

            if not str(chat_id).isdigit() and not chat_id.startswith('@'):
                raise ValueError("is_joined filter error: chat_id must start with '@'")
    ...
    def not_is_joined(*chat_ids: Union[str, int]):
        ...
        if not chat_ids:
            raise ValueError("The 'not_is_joined' filter requires at least one chat ID.")

        for chat_id in chat_ids:
        
            if not str(chat_id).isdigit() and not chat_id.startswith('@'):
                raise ValueError("not_is_joined filter error: chat_id must start with '@'")
    ...
    def callback_query(button_data : str = None):
        ...
        def _filter(message: Update):
            ...
            return message.callback_query is not None and message.callback_query.data == button_data
    ...
    def contact(phone_number : str = None):
        ...
        def _filter(message:Update):
            ...
            return message.contact is not None and message.contact.phone_number == phone_number
    ...
    def entity():
        ...
        def _filter(message:Update):
            ...
            return message.entities is not None
    ...
    def is_admin():
        ...
        async def _filter(update:Update):
            ...
            response = await bot.get_chat_member(update.chat_id, user_id)
    ...
    def user_id(id : Union[str, int]):
        ...
        if not str(id).isdigit():
            ...
            raise ValueError("An error occurred in the user_id filter: ID must be numeric.")
    ...
    def chat_id(id : Union[str, int]):
        ...
        if not str(id).isdigit():
            ...
            raise ValueError("An error occurred in the chat_id filter: ID must be numeric.")
    ...
    def command(command: str = None):
        ...
        if command is not None and command.startswith('/'):
            ...
            raise ValueError("Invalid command format. Remove '/' from the beginning of the command name.")
        ...
        def _filter(update : Update):
            ...
            return text.startswith(f"/{command} ") or text == f"/{command}"
