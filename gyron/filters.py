...
class filters:
    ...
    def is_joined(*chat_ids: Union[str, int]):
        ...
        if not chat_ids:
            ...
            raise ValueError(
                "The 'is_joined' filter requires at least one chat ID.")

        for chat_id in chat_ids:

            if not str(chat_id).isdigit() and not chat_id.startswith('@'):
                raise ValueError(
                    "is_joined filter error: chat_id must start with '@'")
    ...
    def not_is_joined(*chat_ids: Union[str, int]):
        ...
        if not chat_ids:
            raise ValueError(
                "The 'not_is_joined' filter requires at least one chat ID.")

        for chat_id in chat_ids:

            if not str(chat_id).isdigit() and not chat_id.startswith('@'):
                raise ValueError(
                    "not_is_joined filter error: chat_id must start with '@'")
    ...
    def user_id(id: Union[str, int]):
        ...
        if not str(id).isdigit():
            ...
            raise ValueError(
                "An error occurred in the user_id filter: ID must be numeric.")
    ...
    def chat_id(id: Union[str, int]):
        ...
        if not str(id).isdigit():
            ...
            raise ValueError(
                "An error occurred in the chat_id filter: ID must be numeric.")
    ...
    def command(command: str = None):
        ...
        if command is not None and command.startswith('/'):
            ...
            raise ValueError(
                "Invalid command format. Remove '/' from the beginning of the command name.")
