...
class BotClient:
    __slots__ = ('token', 'base_url', 'offset', 'handler',
                 ...
                 'download_base_url', "_middlewares",
                 'global_queue', 'user_queue_maxsize', 'limiter')

    def __init__(self, token: str, use_webhook: bool = False,
                 global_queue_maxsize: int = 5000, user_queue_maxsize: int = 20,
                 limit: int = 20, limit_rate: int = 20):
        ...
        self.user_queue_maxsize = user_queue_maxsize

        self.limiter = Limiter(limit=limit, rate=limit_rate)

    def on_update(self, *filter_func):

        def decorator(user_function):

            data = {'func': user_function, 'filter': {}}
            ...
            for j in filter_func:
                ...
                if asyncio.iscoroutinefunction(j):
    ...
    async def restrict_member(
        ...
        if use_independent_chat_permissions is not None:
            data['use_independent_chat_permissions'] = use_independent_chat_permissions
    ...
    async def promote_member(self, chat_id: Union[str, int], user_id: int,
                             can_change_info: bool = None, can_post_messages: bool = None,
                             can_edit_messages: bool = None, can_delete_messages: bool = None,
                             can_manage_video_chats: bool = None, can_invite_users: bool = None,
                             can_restrict_members: bool = None) -> ApiResponse:
    ...
    async def _extract_user_id(self):

        while True:

            raw = await self.global_queue.get()

            update = Update(raw, self)
            ...
            try:
                ...
                if queue is None:
                    ...
                    queue = asyncio.Queue(self.user_queue_maxsize)
                ...
                try:
                    ...
                    print(f"Update dropped: user {user_id} queue is full")
    ...
    async def _worker(self, user_id):
        ...
        while True:
            queue : asyncio.Queue = self._queues.get(user_id)
            if queue is None:
            ...
            try:
                ...
                print(
                    f"Unexpected error while processing update for user {user_id}: {e}")
    ...
    async def _process_update(self, update: Update):
        ...
        token = self._current_message.set({'chat_id': update.chat_id,
            ...
            for handler_info in self.handler:
                ...
                if filter_func:
                    ...
                    try:
                        ...
                        for f, type in filter_func.items():
                        ...
                        print(
                            f"An exception occurred while evaluating a filter: {e}")
                        should_run = False
                        break
                ...
                try:
                    ...
                    print(
                        f"Unhandled exception while executing handler '{handler_func.__name__}': {e}")
    ...
    async def run(self, timeout: int = 15, sleep: float = 0.7,
        ...
        asyncio.create_task(self._extract_user_id())
        ...
        if self.use_webhook is not True:
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        print(f"The request to the Bale API timed out: {e}")
                        await asyncio.sleep(2)

                    except httpx.HTTPError as e:
                        print(
                            f"An HTTP error occurred while communicating with the Bale API: {e}")
                        await asyncio.sleep(2)

                    except Exception as e:
                        print(f"Unexpected error while fetching updates: {e}")
                        await asyncio.sleep(2)
            ...
            if not webhook_path.startswith('/'):
                ...
                raise ValueError(
                    "Invalid webhook_path: it must start with '/'.")
            ...
            async def run_server(request: Request):

                data = await request.json()

                await self.global_queue.put(data)
            ...
            try:
                ...
                print(
                    f"An error occurred while running the webhook server: {e}")
